...
def process_transcript_with_perplexity(transcript_path, api_key, custom_prompt=None, output_path=None):
    """
    Procesa un archivo de transcripción usando la API de Perplexity
    y guarda el resultado en un nuevo archivo.
    
    Args:
        transcript_path (str): Ruta del archivo de transcripción
    ...
    """
    # Verificar que el archivo de transcripción existe
    ...
    if not custom_prompt:
        custom_prompt = """
        Tienes una transcripción de una asamblea que necesita ser formateada profesionalmente como un acta formal. 
        Por favor, organiza la información en el siguiente formato:
        
        # ACTA DE ASAMBLEA
        
        ## Fecha y Hora
        [Extrae la fecha y hora de la transcripción o usa un formato como "Fecha: [fecha], Hora de inicio: [hora], Hora de finalización: [hora]"]
        
        ## Asistentes
        [Lista de participantes mencionados en la transcripción]
        ...
        ## Cierre
        [Información sobre el cierre de la sesión]
        
        Usa un tono formal y profesional. No agregues información que no esté presente en la transcripción original.
    ...
    payload = {
        "model": "llama-3-sonar-small-32k-online",  # Modelo de Perplexity (actualizar según disponibilidad)
        "messages": [
            {
                "role": "system",
        ...
        "temperature": 0.5,  # Valor más bajo para resultados más consistentes
        "max_tokens": 4000
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Procesa una transcripción con la API de Perplexity')
    parser.add_argument('transcript_path', help='Ruta del archivo de transcripción')
    ...
    try:
        ...
        print(f"Procesamiento completado con éxito. Archivo guardado en: {output_file}")
        sys.exit(0)
    except Exception as e:
